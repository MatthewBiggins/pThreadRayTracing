...
void *calculateRays(void *args){

   long rank = ((pthreadArgs *)args)->threadNum;

   long x, y;

   long start = rank * (HEIGHT/threads);
   long end = ((rank + 1) * (HEIGHT/threads));
   ...
   for(y=start;y<end;y++){
      for(x=0;x<WIDTH;x++){
         ...
         ((pthreadArgs *)args)->r.start.x = x;
         ((pthreadArgs *)args)->r.start.y = y;
         ((pthreadArgs *)args)->r.start.z = -2000;
         
         ((pthreadArgs *)args)->r.dir.x = 0;
         ((pthreadArgs *)args)->r.dir.y = 0;
         ((pthreadArgs *)args)->r.dir.z = 1;
         ...
         do{
            ...
            unsigned int i;
            for(i = 0; i < 3; i++){
               if(intersectRaySphere(&((pthreadArgs *)args)->r, &((pthreadArgs *)args)->spheres[i], &t))
                  currentSphere = i;
            }
            if(currentSphere == -1) break;
            
            vector scaled = vectorScale(t, &((pthreadArgs *)args)->r.dir);
            vector newStart = vectorAdd(&((pthreadArgs *)args)->r.start, &scaled);
            
            /* Find the normal for this new vector at the point of intersection */
            vector n = vectorSub(&newStart, &((pthreadArgs *)args)->spheres[currentSphere].pos);
            ...
            material currentMat = ((pthreadArgs *)args)->materials[((pthreadArgs *)args)->spheres[currentSphere].material];
            
            /* Find the value of the light at this point */
            unsigned int j;
            for(j=0; j < 3; j++){
               light currentLight = ((pthreadArgs *)args)->lights[j];
               ...
               unsigned int k;
               for (k = 0; k < 3; ++k) {
                  if (intersectRaySphere(&lightRay, &((pthreadArgs *)args)->spheres[k], &t)){
                     inShadow = true;
                     break;
                  }
               }
            ...
            ((pthreadArgs *)args)->r.start = newStart;
            float reflect = 2.0f * vectorDot(&((pthreadArgs *)args)->r.dir, &n);
            vector tmp = vectorScale(reflect, &n);
            ((pthreadArgs *)args)->r.dir = vectorSub(&((pthreadArgs *)args)->r.dir, &tmp);

            level++;

         }while((coef > 0.0f) && (level < 15));
         
         ((pthreadArgs *)args)->img[(x + y*WIDTH)*3 + 0] = (unsigned char)min(red*255.0f, 255.0f);
         ((pthreadArgs *)args)->img[(x + y*WIDTH)*3 + 1] = (unsigned char)min(green*255.0f, 255.0f);
         ((pthreadArgs *)args)->img[(x + y*WIDTH)*3 + 2] = (unsigned char)min(blue*255.0f, 255.0f);
...
int main(int argc, char *argv[]){
   ...
   for (threadNum = 0; threadNum < threads; threadNum++){
      arguments = malloc(sizeof(pthreadArgs));
      arguments->img = img;
      arguments->r = r;
      memcpy(arguments->lights, lights, sizeof(light) * 3);
      memcpy(arguments->materials, materials, sizeof(material) * 3);
      memcpy(arguments->spheres, spheres, sizeof(sphere) * 3);
      arguments->threadNum = threadNum;
      pthread_create(&thread_handles[threadNum], NULL, calculateRays, (void*) arguments);
   }

   for (threadNum = 0; threadNum < threads; threadNum++){
      pthread_join(thread_handles[threadNum], NULL);
   }
   ...
   if (output != 0)
      saveppm("image.ppm", arguments->img, WIDTH, HEIGHT);
